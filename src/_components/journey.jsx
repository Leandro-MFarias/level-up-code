...
export function Journey() {
  const { data: lists, isLoading } = useList();
      ...
      {lists.map((list) => (
        <div key={list.id} className="w-full px-10">
          ...
          <div className="space-y-2">
            {list?.exerciciosIds.map((exercicio) => (
              <div
                key={exercicio}
                className="flex flex-col items-center space-y-4.5"
              >
                <div className="relative">
                  <Link to={`/exercises/${exercicio}`}>
                    <button
                      className={`btn relative flex h-24 w-24 cursor-pointer items-center justify-center rounded-[50%] ${list === true ? "completed bg-lime-500" : "blocked bg-zinc-600"}`}
                    >
