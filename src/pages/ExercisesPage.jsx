...
export function ExercisesPage() {
  ...
  async function handleSubmit() {
    if (!selected) {
      setFeedback("Selecione uma alternativa antes de enviar!");
      setIsCorrect(null);
      return;
    }
  ...
  return (
    <div className="mx-auto flex max-w-7xl flex-col space-y-16 overflow-hidden">
      ...
      <div className="flex h-[50vh] flex-col justify-between">
        <div className="flex flex-col space-y-4">
          <h1 className="text-3xl font-bold">{formater(exercise.tipo)}:</h1>
          <p className="pl-14 text-2xl">{exercise.pergunta}</p>
        </div>
        <div className="flex justify-center space-x-6">
          {exercise.alternativas.map((alternativa, index) => {
            // Define a cor baseada no estado atual
            let buttonClasses =
              "border-2 rounded-md h-12 w-56 cursor-pointer transition duration-150 ease-in-out ";
      ...
      <div
        className={`flex items-center space-x-20 px-4 ${
          isCorrect !== null ? "justify-center" : "justify-end"
        }`}
      >
        {feedback && (
          <div
            className={`flex flex-col space-y-2 text-center text-xl font-semibold ${
              isCorrect === true
                ? "text-lime-500"
                : isCorrect === false
                  ? "text-red-600"
